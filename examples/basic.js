 ...
 */
const sum = delay(1000, 10).then(x =>
  delay(1000, 20).then(y => Task.of(x + y))
);
...
sum.run(logResolve("sum"), logReject("sum"), logCancel("sum"));

/**
 * if any task's step aborts, the whole task aborts
 */
const sumErr = delay(1000, 10).then(x =>
  delay(1000, 20).then(y =>
    /**
   * task will fail here
   */
    Task.reject("sum error").then(_ => Task.of(x + y))
  )
);
 ...
 */
const sumCancel = delay(1000, 10).then(x =>
  delay(1000, 20).then(y =>
